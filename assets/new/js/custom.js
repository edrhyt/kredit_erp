...
for (var j = 0; j < i; j++) {
    $('#addr' + j ).childern('#harga_brg,#jml_brg').keyup(function() {
        var harga_brg   = parseInt($('#addr' + j ).childern('#harga_brg').val());
        var jml_brg     = parseInt($('#addr' + j ).childern('#jml_brg').val());
        var jum         = harga_brg * jml_brg ;
        $('#addr' + j ).childern('#jum').val(jum);
    });
}
//
var room = 1;
